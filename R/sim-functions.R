...
#' @keywords internal
.new_point_process_sim <- function(
  events,
  T_max,
  params,
  type,
  ...
  if (!is.null(marks)) {
    class_vector <- c("marked_pp_sim", class_vector)
  }
...
sim_pp <- function(T_max, intens, M = NULL) {
  ...
  if (is.numeric(intens)) {
    stopifnot(
      "'T_max' must be positive" = is.numeric(T_max) &&
        length(T_max) == 1 &&
        T_max > 0,
      "'intens' must be a single non-negative number" = length(intens) == 1 &&
        !is.na(intens) &&
        intens >= 0
    )
    ...
    stopifnot(
      ...
      "Inhomogeneous simulation requires a single numeric 'M' >= 0" = is.numeric(
        M
      ) &&
        length(M) == 1 &&
        !is.na(M) &&
...
sim_hp <- function(T_max, lambda, mu) {
  # some guardrails
  stopifnot(
    "'T_max' must be a single numeric" = is.numeric(T_max) &&
      length(T_max) == 1 &&
      !is.na(T_max),

    "'T_max' must be positive" = T_max > 0,

    "'lambda' must be a single numeric" = is.numeric(lambda) &&
      length(lambda) == 1 &&
      !is.na(lambda),

    "'lambda' must be non-negative" = lambda >= 0,

    "'mu' must be a function" = is.function(mu)
  )
...
sim_hp_clus <- function(T_max, lambda, eta, family, ...) {
  ...
  stopifnot(
    ...
    "'eta' (branching ratio) must be a single numeric" = is.numeric(eta) &&
      length(eta) == 1 &&
      !is.na(eta),

    "'eta' must be non-negative" = eta >= 0,

    "'family' must be a single character string" = is.character(family) &&
      length(family) == 1
  )
...
sim_mhp <- function(T_max, intensity_func, rmark_func, ...) {
  ...
  while (t < T_max) {
    ...
    if (is.na(M)) {
      stop("Intensity calculation returned NA.")
    }
...
print.marked_pp_sim <- function(x, ...) {
  ...
  if (x$n > 0) {
    ...
    if (is.numeric(x$marks)) {
      ...
      cat(
        "        First few: ",
        paste(round(utils::head(x$marks, 3), 3), collapse = ", "),
        "...\n"
      )
    } else {
      cat(class(x$marks)[1], "\n")
      cat(
        "        First few: ",
        paste(utils::head(x$marks, 3), collapse = ", "),
        "...\n"
      )
    }
  }
